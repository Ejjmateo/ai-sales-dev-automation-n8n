...
Receives lead data from a form/CRM webhook
Uses an AI research agent to identify the official company website and domain
Escalates to a human in Slack if the company match confidence is low
Enriches the company profile using the Apollo API
Passes enriched data into an AI qualification agent
Scores the lead 1–10 and classifies it as High, Moderate, or Low quality
Routes leads to three dedicated Slack channels based on quality tier
Generates a personalized outreach email draft for high-priority leads
Saves the draft to Gmail, notifies the sales team in Slack
Logs enriched lead data and outreach status into Google Sheets
Surfaces all pipeline activity in a live React monitoring dashboard
...
SectionDetailsMetric cardsTotal leads · High / Moderate / Low counts · Draft rate %Score distributionBar chart — lead counts by score bucket (1–3, 4–6, 7–10)Quality breakdownDonut chart — High / Moderate / Low split by percentageDraft conversionBar chart by industry — drafted vs not draftedPipeline routing3-lane visualization mapped to Slack channels with live countsLead tableFilterable by quality tier · Sortable by score · AI reasoning per row
...
Tech Stack
LayerToolsWorkflow orchestrationn8nLead intakeGoHighLevel / form webhookCompany enrichmentApollo APIWeb researchTavily Search APIAI agentsOpenRouter (Grok 4.1 Fast)Human-in-the-loopSlackOutreachGmail (draft creation)CRM loggingGoogle SheetsDashboard frontendReact + Vite + Recharts + TailwindDashboard data sourceGoogle Sheets API v4
...
Make the Google Sheet public: Share → Anyone with the link → Viewer
Create a Google Cloud project, enable the Sheets API, and generate an API key
Restrict the key to HTTP referrers and the Sheets API only
Add the key to dashboard/.env as VITE_GOOGLE_SHEETS_API_KEY
Update the spreadsheet ID in dashboard/src/data/sheets.js
...
Portfolio: eljonmateo.dev
GitHub: github.com/Ejjmateo
